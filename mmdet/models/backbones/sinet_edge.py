...
class RF(BaseModule):
    ...
    def __init__(self, in_channel, out_channel, init_cfg=None):
        super(RF, self).__init__(init_cfg)
...
class convlayer(BaseModule):
    def __init__(self, in_channels, out_channels,
                 ...
                 norm_cfg=dict(type='BN'),
                 init_cfg=None):
        super(convlayer, self).__init__(init_cfg=init_cfg)
        self.relu = nn.ReLU(True)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        convs = nn.Conv2d(2, 1, 7, padding=3, bias=False)
        x = convs(x)
...
class SiEdgenet(BaseModule):
    ...
    def __init__(self,
        ...
        self.edge = convlayer(self.base_channels, self.channel, init_cfg=None)
        self.downSample = nn.MaxPool2d(2, stride=2)
    ...
    def forward(self, x):
        ...
        # x1 = torch.mul(x1, self.edge(edge_x0)).float()
        ...
        x2_sm_cat = torch.cat((x2_sm, upsample_2(x3_sm), upsample_2(upsample_2(x4_sm))), dim=1)
        x3_sm_cat = torch.cat((x3_sm, upsample_2(x4_sm)), dim=1)
