...
class MotionBlur(object):
    ...
    def motion_blur(self, img, img_shape):
        ...
        """
        if img_shape[1] <= 750:
            kernel_size = int(random.choice(range(10, 20), 1)[0])
        elif img_shape[1] < 2000 and img_shape[1] > 750:
            kernel_size = int(random.choice(range(20, 60), 1)[0])
        elif img_shape[1] >= 2000:
            kernel_size = int(random.choice(range(100, 140), 1)[0])
...
class Pad:
    ...
    def _pad_img(self, results):
        ...
        if 'img_retinex' in results:
            if self.pad_to_square:
                max_size = max(results['img_retinex'].shape[:2])
                self.size = (max_size, max_size)
            if self.size is not None:
                padded_img = mmcv.impad(
                    results['img_retinex'], shape=self.size, pad_val=pad_val)
            elif self.size_divisor is not None:
                padded_img = mmcv.impad_to_multiple(
                    results['img_retinex'], self.size_divisor, pad_val=pad_val)
            results['img_retinex'] = padded_img
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            results[key] = mmcv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
        # if 'img_retinex' in results:
        #     results['img_retinex'] = mmcv.imnormalize(results['img_retinex'], self.mean, self.std, self.to_rgb)
...
class UnderwaterRestoration:
    ...
    def __init__(self,
        ...
        if isinstance(ratio, list):
            ...
            assert 0 <= sum(ratio) <= 1
        elif isinstance(ratio, float):
            assert 0 <= ratio <= 1
        elif ratio is None:
            pass
        else:
            raise ValueError('ratios must be None, float, '
                             'or list of float')
        self.ratio = ratio
    ...
    def transmissionestimate(im, A, sz):
        ...
        transmission = 1 - omega * UnderwaterRestoration.darkchannel(im3, sz)
        return transmission
    ...
    def transmissionrefine(im, et):
        ...
        r = 60
        eps = 0.0001
        t = UnderwaterRestoration.guidedfilter(gray, et, r, eps)
    ...
    def __call__(self, results):
        ...
        if random.random() < self.ratio:
            ...
            dark = self.darkchannel(I, self.channel)
            A = self.atmlight(I, dark)
            te = self.transmissionestimate(I, A, 15)
            t = self.transmissionrefine(img, te)
