...
#                               kernel_size=kernel_size, stride=stride,
#                               padding=padding, dilation=dilation, bias=False, groups=in_planes)
#         self.pointwiseconv = nn.Conv2d(in_planes, out_planes,
...
class edge_attention_song(nn.Module):
    def __init__(self,inchannel,out_channel):
        super(edge_attention_song,self).__init__()
...
class PDC_SM(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv_concat3 = BasicConv2d(channel, 4*channel, 3, padding=1)
        self.conv4 = BasicConv2d(4*channel, 4*channel, 3, padding=1)
        self.conv5 = nn.Conv2d(4*channel, 256, 3, padding=1)
...
class ContextBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.channel_add_conv is not None:
        #     # [N, C, 1, 1]
        #     channel_add_term = self.channel_/add_conv(context)
        #     out = out + channel_add_term
        return out

@BACKBONES.register_module()
class SiEdgenet(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SiEdgenet, self).__init__(init_cfg)
        ...
        self.rf_low_sm = RF(320, self.channel)
        self.rf2_sm = RF(3584, 512)
        self.rf3_sm = RF(3072, 1024)
        self.pdc_sm = PDC_SM(self.channel)
        self.upsample_2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.upsample_8 = nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True)
        self.SA = SA()
        self.avgpool = nn.AvgPool2d(7)

        self.edge1 = edge_attention(64, 256)
    ...
    def forward(self, x):
        ...
        edge_x0 = DeepSobel_song(x)
        # heatmap = cv2.applyColorMap(np.uint8(255 * edge_x0.cpu().numpy()), cv2.COLORMAP_JET)
        # cv2.imwrite("heatmap.jpg", heatmap)
        # edge_x2 =DeepSobel(x2)
        edge_x1 = self.edge1(edge_x0)
        # edge_x2 = self.edge2(edge_x0)
        # edge_x3 = self.edge3(edge_x0)

        # x2_edge = getattr(self, 'layer2')(edge)
        # edge = torch.from_numpy(edge).cuda()
        x1 = torch.mul(x1, edge_x1).float()

        # x2 = torch.mul(x2, self.downSample(edge_x2)).float()
        # del edge
        x01 = torch.cat((x, x1), dim=1)
        # first output
        x01_sm_rf = self.rf_low_sm(x01)

        x2_sm = x2
        x3_sm = getattr(self, 'layer3')(x2_sm)
        # x3_sm = torch.mul(x3_sm, edge_x3).float()
        ...
        x2_sm_cat = torch.cat((x2_sm, self.upsample_2(x3_sm), self.upsample_2(self.upsample_2(x4_sm))), dim=1)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(SiEdgenet, self).train(mode)
        self._freeze_stages()
